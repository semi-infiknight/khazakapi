...
/** Spread same-vendor logos across rows so scrolling tracks do not cluster duplicates. */
...
function mergeRowTiles(primary, secondary) {
  const row = [];
  const buckets = [primary, secondary].filter((bucket) => bucket.length);
  ...
  while (row.length < total) {
    ...
    for (let attempt = 0; attempt < buckets.length; attempt += 1) {
      const index = (cursor + attempt) % buckets.length;
      const bucket = buckets[index];
      if (!bucket.length) continue;
      if (buckets.some((entry, entryIndex) => entryIndex !== index && entry.length)) {
        const tail = row[row.length - 1];
        if (tail && vendorKey(tail.slug) === vendorKey(bucket[0].slug)) continue;
      }
      row.push(bucket.shift());
      cursor = (index + 1) % buckets.length;
      picked = true;
      break;
    }

    if (!picked) {
      const fallback = buckets.find((bucket) => bucket.length);
      row.push(fallback.shift());
    }
  }
...
export function buildArchWallRows(tiles, rowCount = KZ_ARCH_ROW_COUNT) {
  const yandexTiles = tiles.filter((tile) => vendorKey(tile.slug) === "yandex");
  const otherTiles = interleaveTilesByVendor(tiles.filter((tile) => vendorKey(tile.slug) !== "yandex"));
  const rows = Array.from({ length: rowCount }, () => ({ yandex: [], other: [] }));

  yandexTiles.forEach((tile, index) => {
    rows[index % rowCount].yandex.push(tile);
  });

  otherTiles.forEach((tile, index) => {
    rows[index % rowCount].other.push(tile);
  });

  const built = rows.map(({ yandex, other }) => mergeRowTiles(yandex, other));
  polishRowVendorRuns(built);
  return built;
}
